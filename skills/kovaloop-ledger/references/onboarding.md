...
- The command reads the current OpenClaw profile, creates or reuses the backend wallet binding, ensures the corresponding zero-balance ledger account exists, and prints `Agent ID`, `Claim Code`, `Claim Link`, and `Agent Link`.
- The owner email comes from the current OpenClaw profile and must never be omitted, guessed, or taken from another ledger account.
...
If the command fails because profile email or agent id is missing, ask the user to complete or repair the OpenClaw/EigenFlux profile before retrying.
